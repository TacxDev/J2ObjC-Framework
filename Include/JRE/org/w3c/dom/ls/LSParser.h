...
//  source: android/platform/libcore/luni/src/main/java/org/w3c/dom/ls/LSParser.java
//
...
#define INCLUDE_ALL_OrgW3cDomLsLSParser 0
#else
#define INCLUDE_ALL_OrgW3cDomLsLSParser 1
#endif
...
/*!
 @brief The <code>DOMConfiguration</code> object used when parsing an input
  source.This <code>DOMConfiguration</code> is specific to the parse
  operation.
 No parameter values from this <code>DOMConfiguration</code>
 ...
 <code>LSResourceResolver</code> referenced in this parameter resolves
  ...
  type. If an unsupported media type is encountered, a fatal error of type 
 <b>"unsupported-media-type"</b> will be raised. The media types defined in [<a href='http://www.ietf.org/rfc/rfc3023.txt'>IETF RFC 3023</a>] must always
  be accepted. </dd>
  <dt><code>false</code></dt>
...
/*!
 @brief When a filter is provided, the implementation will call out to the
...
/*!
 @brief <code>true</code> if the <code>LSParser</code> is asynchronous, 
 <code>false</code> if it is synchronous.
 */
- (jboolean)getAsync;

/*!
 @brief <code>true</code> if the <code>LSParser</code> is currently busy
  loading a document, otherwise <code>false</code>.
 */
- (jboolean)getBusy;

/*!
 @brief Parse an XML document from a resource identified by a 
 <code>LSInput</code>.
...
/*!
 @brief Parse an XML document from a location identified by a URI reference [<a href='http://www.ietf.org/rfc/rfc2396.txt'>IETF RFC 2396</a>].If the URI
...
/*!
 @brief Parse an XML fragment from a resource identified by a 
 ...
 <code>action</code> arguments.When parsing the input stream, the
  ...
  inserted) is used for resolving unbound namespace prefixes.
 The
 ...
 <code>true</code>).
  <br> If an error occurs while parsing, the caller is notified through the 
 <code>ErrorHandler</code> instance associated with the "<a href='http://www.w3.org/TR/DOM-Level-3-Core/core.html#parameter-error-handler'>
 ...
 @param input The  <code> LSInput </code>  from which the source document
     is to be read. The source document must be an XML fragment, i.e.    anything except a complete XML document (except in the case where
     the context node of type 
  <code> DOCUMENT_NODE </code> , and the action    is  <code>
  ACTION_REPLACE_CHILDREN </code> ), a DOCTYPE (internal    subset), entity declaration(s), notation declaration(s), or XML or
     text declaration(s).
 @param contextArg The node that is used as the context for the data    that is being parsed. This node must be a 
 ...
 @param action This parameter describes which action should be taken    between the new set of nodes being inserted and the existing
...
/*!
 @brief Abort the loading of the document that is currently being loaded by the 
 <code>LSParser</code>.If the <code>LSParser</code> is currently
 ...
 */
inline jshort OrgW3cDomLsLSParser_get_ACTION_APPEND_AS_CHILDREN(void);
 ...
 */
inline jshort OrgW3cDomLsLSParser_get_ACTION_REPLACE_CHILDREN(void);
...
/*!
 ...
 <code>DocumentFragment</code>.
 */
inline jshort OrgW3cDomLsLSParser_get_ACTION_INSERT_BEFORE(void);
...
/*!
 ...
 <code>DocumentFragment</code>.
 */
inline jshort OrgW3cDomLsLSParser_get_ACTION_INSERT_AFTER(void);
...
/*!
 @brief Replace the context node with the result of the parse operation.For
  this action to work, the context node must have a parent, and the
  parent must be an <code>Element</code> or a 
 <code>DocumentFragment</code>.
 */
inline jshort OrgW3cDomLsLSParser_get_ACTION_REPLACE(void);
