...
/*!
 @brief Attempts to strip RTL, LTR and Arabic letter markers from <code>symbol</code>.
 If the string contains a single non-marker character (and any number of marker characters),
  then that character is returned, otherwise <code>fallback</code> is returned.
 */
+ (jchar)maybeStripMarkersWithNSString:(NSString *)symbol
                              withChar:(jchar)fallback;
...
FOUNDATION_EXPORT jchar JavaTextDecimalFormatSymbols_maybeStripMarkersWithNSString_withChar_(NSString *symbol, jchar fallback);
