...
/*!
 ...
 <p>A transition between two offsets is normally the result of a daylight savings cutover. The
  discontinuity is normally a gap in spring and an overlap in autumn. <code>ZoneOffsetTransition</code>
  models the transition between the two offsets. 
 <p>Gaps occur where there are local date-times that simply do not exist. An example would be when
  the offset changes from <code>+03:00</code> to <code>+04:00</code>. This might be described as 'the clocks
  will move forward one hour tonight at 1am'. 
 <p>Overlaps occur where there are local date-times that exist twice. An example would be when the
  offset changes from <code>+04:00</code> to <code>+03:00</code>. This might be described as 'the clocks will
...
/*!
 ...
 <p>This is the first date-time after the discontinuity, when the new offset applies. 
 <p>The combination of the 'before' date-time and offset represents the same instant as the
...
/*!
 ...
 <p>This is the date-time where the discontinuity begins expressed with the 'before' offset. At
...
/*!
 ...
 <p>In most cases, the transition duration is one hour, however this is not always the case. The
...
/*!
 ...
 <p>This is the instant of the discontinuity, which is defined as the first instant that the
  'after' offset applies. 
 <p>The methods <code>getInstant()</code>, <code>getDateTimeBefore()</code> and <code>getDateTimeAfter()</code>
...
/*!
 ...
 <p>Gaps occur where there are local date-times that simply do not exist. An example would be
  when the offset changes from <code>+01:00</code> to <code>+02:00</code>. This might be described as 'the
...
/*!
 ...
 <p>Overlaps occur where there are local date-times that exist twice. An example would be when
  the offset changes from <code>+02:00</code> to <code>+01:00</code>. This might be described as 'the
...
/*!
 ...
 <p>This checks to see if the given offset will be valid at some point in the transition. A gap
...
/*!
 ...
 @throw IllegalArgumentExceptionif <code>offsetBefore</code> and <code>offsetAfter</code> are equal, or
